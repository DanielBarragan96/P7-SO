...
int getPag_a_expulsar(int* frames){
	int pag_exp = 0;
	if(ptbr[frames[0]].tlastaccess < ptbr[frames[1]].tlastaccess && ptbr[frames[0]].tlastaccess < ptbr[frames[2]].tlastaccess){
            pag_exp = frames[0];//the frame 0 is the one to move to virtual memory
        }
        else if(ptbr[frames[1]].tlastaccess < ptbr[frames[0]].tlastaccess && ptbr[frames[1]].tlastaccess < ptbr[frames[2]].tlastaccess){
            pag_exp = frames[1];//the frame 1 is the one to move to virtual memory
        }
        else{
            pag_exp = frames[2];//the frame 2 is the one to move to virtual memory
	}
	return pag_exp;		
}
...
////////checar cual de los access fue el menos reciente
       pag_a_expulsar = getPag_a_expulsar(&frames[0]);
